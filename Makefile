...
KIND_CLUSTER ?= aks-operator-test-e2e
...
SBOM_PROJECT_NAME ?= aks-operator
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name aks-operator-builder
	$(CONTAINER_TOOL) buildx use aks-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm aks-operator-builder
	rm Dockerfile.cross
